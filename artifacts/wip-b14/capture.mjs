// OW-B14 board capture: lock 14 (The Bind-Rune Seal) at both viewports, dSF2,
// plus the density rubric numbers (field occupancy, largest featureless region,
// board->controls gap) and the 44px target floor.
// Usage: node artifacts/wip-b14/capture.mjs <tag>
import { writeFileSync, mkdirSync } from 'node:fs';
import * as H from '../wip-qplay/harness.mjs';

const tag = process.argv[2] || 'now';
mkdirSync('artifacts/wip-b14/shots', { recursive: true });

// The shared harness's crossThreshold predates the threshold wager card
// (src/shell/screens/threshold.js showWager, landed by another lane). The rig
// keeps its own passer rather than editing the shared harness.
...
for (const [name, vp] of [['desk', H.DESKTOP], ['phone', H.PHONE]]) {
  ...
  await page.waitForTimeout(1500); // mid-showing: the ghost hand on the ice-rune
  await page.screenshot({ path: `artifacts/wip-b14/shots/${tag}-${name}-demo.png` });
  await page.waitForTimeout(2300); // let the showing run out
  await page.screenshot({ path: `artifacts/wip-b14/shots/${tag}-${name}.png` });
  ...
  const m = await page.evaluate(() => {
    ...
    const act = root.querySelector('.btn-carved, button.ow14-act');
    let gap = null;
    if (act) {
  ...
  // ---- live overlay proof: two bound runes + the ice-rune (idle) + a
  // one-stroke-short trap (astray), then the full answer -------------------
  ...
  const click = async (ch) => root.locator('.ow14-cand').nth(instance.candidates.indexOf(ch)).click();
  const trap = instance.candidates.find((c) => !answer.runes.includes(c) && c !== 'ᛁ' && c !== 'ᚢ' && c !== 'ᛋ');
  for (const ch of answer.runes.slice(0, 2)) await click(ch);
  await click('ᛁ');
  await click(trap);
  await page.waitForTimeout(260);
  await page.screenshot({ path: `artifacts/wip-b14/shots/${tag}-${name}-mid.png` });
  m.midStatus = await root.locator('p[aria-live="polite"]').innerText();
  m.midTally = await root.locator('.ow14-count').innerText();
  await click('ᛁ');
  await click(trap);
  for (const ch of answer.runes.slice(2)) await click(ch);
  await page.waitForTimeout(260);
  m.fullTally = await root.locator('.ow14-count').innerText();
  await page.screenshot({ path: `artifacts/wip-b14/shots/${tag}-${name}-full.png` });

  m.consoleErrors = page.__errors.slice();
  out.push({ vp: name, ...m });
  await page.context().close();
}
// reduced-motion: the same showing, held still
{
  ...
  await page.screenshot({ path: `artifacts/wip-b14/shots/${tag}-calm-demo.png` });
  const calm = await page.evaluate(() => {
    const root = document.querySelector('.lock-root');
    const fireAnim = getComputedStyle(root.querySelector('.ow14-fire')).animationName;
    const ghost = root.querySelector('.ow14-ghost');
    return {
      fireAnim,
      ghostShown: !!ghost && ghost.style.display !== 'none' && !!ghost.querySelector('canvas'),
      tally: root.querySelector('.ow14-count').textContent,
      motions: typeof root.getAnimations === 'function' ? root.getAnimations().length : -1,
    };
  });
  await page.waitForTimeout(2200);
  const after = await page.evaluate(() => document.querySelector('.lock-root .ow14-count').textContent);
  console.log(`calm: fireAnim=${calm.fireAnim} ghostShown=${calm.ghostShown} during="${calm.tally}" after="${after}"`);
  await page.context().close();
}
await browser.close();
writeFileSync(`artifacts/wip-b14/shots/${tag}-metrics.json`, JSON.stringify(out, null, 1));
for (const r of out) {
  ...
  console.log(`   mid: "${r.midTally}" | ${r.midStatus} || full: "${r.fullTally}"`);
