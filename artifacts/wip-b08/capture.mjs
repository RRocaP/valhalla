// OW-B08 board capture: lock 08 (The Twelve Pieces) at both viewports, dSF2,
// plus the density rubric numbers (field occupancy, largest featureless region,
// board->controls gap) and the 44px target floor.
// Usage: node artifacts/wip-b08/capture.mjs <tag>
import { writeFileSync, mkdirSync } from 'node:fs';
import * as H from '../wip-qplay/harness.mjs';

const tag = process.argv[2] || 'now';
mkdirSync('artifacts/wip-b08/shots', { recursive: true });

// The threshold now shows the wager framing card between the begin gesture and
// the lid (src/shell/screens/threshold.js), which the shared harness's
// crossThreshold does not yet know about. Local step; the harness is not ours.
async function cross(page) {
  ...
  if (await wager.count()) {
    await wager.first().waitFor({ state: 'visible', timeout: 6000 });
    await wager.first().click();
  }
...
for (const [name, vp] of [['desk', H.DESKTOP], ['phone', H.PHONE]]) {
  ...
  // the showing (ghost hand + cross-reference glint) runs for its 3s first
  await page.waitForTimeout(900);
  await page.screenshot({ path: `artifacts/wip-b08/shots/${tag}-${name}-showing.png` });
  const demo = await page.evaluate(() => {
    const host = [...document.querySelectorAll('.lock-root div')]
      .find((d) => d.style.zIndex === '3' && d.getAttribute('aria-hidden') === 'true');
    const skip = [...document.querySelectorAll('.ow8-skip')].filter((b) => b.style.display !== 'none');
    return { ghostShown: !!host && host.style.display === 'block', skipShown: skip.length,
      line: (document.querySelector('.ow8-stage') || {}).textContent || '' };
  });
  await page.waitForTimeout(2900); // let the showing run out
  await page.screenshot({ path: `artifacts/wip-b08/shots/${tag}-${name}.png` });
  ...
  const m = await page.evaluate(() => {
    ...
    const act = root.querySelector('.btn-carved, button.ow8-act');
    let gap = null;
    if (act) {
  ...
  // ---- real input: pick a piece, call a direction, then look again ----
  // (the glint cross-reference and the seated-accusation staging only exist
  //  once the board has been touched, so an idle shot cannot judge them)
  const root = page.locator('.lock-root');
  await root.locator('[role="radio"]').nth(4).click();
  await page.waitForTimeout(180);
  await root.getByRole('button', { name: 'heavy — salted', exact: true }).click();
  await page.waitForTimeout(320);
  await page.screenshot({ path: `artifacts/wip-b08/shots/${tag}-${name}-picked.png` });
  m.staged = await page.evaluate(() => ({
    stage: (document.querySelector('.ow8-stage') || {}).textContent || '',
    read: (document.querySelector('.ow8-read') || {}).textContent || '',
    swearEnabled: !document.querySelector('.ow-hacksilver .btn-carved').disabled,
  }));
...
await browser.close();
writeFileSync(`artifacts/wip-b08/shots/${tag}-metrics.json`, JSON.stringify(out, null, 1));
for (const r of out) {
