// OW-B10 board capture: lock 10 (The Dróttkvætt Lines) at both viewports,
// dSF2, plus the docs/QUALITY.md density rubric numbers (field occupancy,
// largest featureless region, board->controls gap) and the 44px target floor.
// Usage: node artifacts/wip-b10/capture.mjs <tag> [seatedTag]
//   tag        capture of the board as it first reads (after the showing)
//   seatedTag  optional second capture with the true verse seated, so the
//              gold/blood law lights are in the frame
import { writeFileSync, mkdirSync } from 'node:fs';
import * as H from '../wip-qplay/harness.mjs';

const tag = process.argv[2] || 'now';
const seatedTag = process.argv[3] || null;
...
const MEASURE = () => {
  ...
  const act = root.querySelector('.btn-carved, .ow-drott .send');
  let gap = null;
  if (act) {
    const ab = act.getBoundingClientRect();
    let bottom = r.top;
    for (const c of root.querySelectorAll('canvas,.ow10-lectern,.tray,.frag')) {
      const b = c.getBoundingClientRect();
...
// Seat the true verse through the real controls, so the captures show the
// metre lights actually firing (this is the whole point of the board).
async function seatTruth(page) {
  const { answer, instance } = await page.evaluate(() => {
    ...
    return { answer: l.solve(inst), instance: { fragments: inst.fragments.map((f) => ({ text: f.text })) } };
  });
  const root = page.locator('.lock-root');
  for (let line = 0; line < answer.lines.length; line++) {
    for (const half of [0, 1]) {
      const text = instance.fragments[answer.lines[line][half]].text;
...
// The threshold gained a wager card after the begin gesture (screens/threshold.js);
// wip-qplay/harness.mjs crossThreshold predates it, so cross it locally.
...
for (const [name, vp] of [['desk', H.DESKTOP], ['phone', H.PHONE]]) {
  ...
  out.push({ vp: name, phase: 'fresh', ...m });

  if (seatedTag) {
    await seatTruth(page);
    await page.waitForTimeout(320);
    await page.screenshot({ path: `artifacts/wip-b10/shots/${seatedTag}-${name}.png` });
    const m2 = await page.evaluate(MEASURE);
    m2.consoleErrors = page.__errors.slice();
    out.push({ vp: name, phase: 'seated', ...m2 });
  }
  await page.context().close();
}
await browser.close();
writeFileSync(`artifacts/wip-b10/shots/${tag}-metrics.json`, JSON.stringify(out, null, 1));
for (const r of out) {
  console.log(`${r.vp}/${r.phase}: box ${r.box.w}x${r.box.h} scrollH=${r.scrollH} occupancy=${r.occupancy} largestVoid=${r.largestVoid} gap=${r.gap} canvases=${r.nCanvas} blank=${r.blankCanvas} under44=${r.under44.length} errs=${r.consoleErrors.length}`);
