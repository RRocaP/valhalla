// The shared artifacts/wip-qplay/ink-targets.mjs floor, run for lock 12 only.
// Identical assertions (blank canvas = ink <= 20 px; every visible interactive
// target >= 44 x 44 CSS px), with the threshold's new wager layer stepped
// through — the shared script's crossThreshold() predates it and times out.
...
for (const vp of [H.DESKTOP, H.PHONE]) {
  ...
  rows.push({ vp: `${vp.width}x${vp.height}`, ord: 12, ...m, errs: page.__errors.length });
  if (m.blank.length || m.under44) bad++;
  await page.context().close();
}
await browser.close();
writeFileSync('artifacts/wip-b12/ink-targets.json', JSON.stringify(rows, null, 1));
